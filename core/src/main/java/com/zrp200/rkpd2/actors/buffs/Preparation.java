...
public class Preparation extends Buff implements ActionIndicator.Action {
	...
	public boolean procKO(Char attacker, Char enemy){
		if (enemy.isAlive() && enemy.alignment != attacker.alignment && canKO(enemy)){
			enemy.HP = 0;
			...
			if (attacker instanceof Hero) {
				...
				if (((Hero) attacker).hasTalent(Talent.DARKENING_STEPS)) {
					Buff.affect(attacker, ArtifactRecharge.class).extend(Dungeon.hero.pointsInTalent(Talent.DARKENING_STEPS) * 2);
				}
			}
			return true;
		}
